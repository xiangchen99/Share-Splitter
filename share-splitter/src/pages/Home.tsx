        ...
        <Card className="mb-6">
          ...
          <div className="space-y-3">
            {bills.map((bill) => (
              ...
              >
                ...
                <Button danger size="small" onClick={() => removeBill(bill.id)}>
                  Remove
                </Button>
              </div>
            ))}
          ...
          <div className="mt-4 pt-4 border-t">
            <div className="flex justify-between items-center">
              ...
              <Text>
                <strong>Bills:</strong> {bills.length}
              </Text>
            </div>
            <Text>
              <strong>Participants:</strong> {users.length}
            </Text>
          </div>
        </Card>
      )}
        ...
        <Card>
          ...
          <div className="space-y-2">
            {totalCalculatedAmounts.map(({ user, totalAmount: userAmount }) => (
              ...
              >
                <span>
                  {user.name} ({user.calculatedPercentage?.toFixed(1)}%)
                </span>
