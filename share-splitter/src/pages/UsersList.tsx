...
function UsersList() {
    ...
    editForm
      ...
      .then((values) => {
        ...
        if (percentage > 0 && dollarAmount > 0) {
          message.error(
            "Please enter either a percentage OR a dollar amount, not both"
          );
          return;
        }
  ...
  const remainingDollarAmount = Math.max(
    0,
    totalAmount -
      totalFixedDollarAmount -
      (totalAmount * totalFixedPercentage) / 100
  );
      ...
      <Card className="mb-6">
        <div className="grid gap-4">
          ...
          <div className="text-sm text-gray-600">
            Leave both empty for flexible split, or enter either percentage OR
          ...
          <Card key={user.id} className="flex justify-between items-center">
            <div className="flex justify-between items-center w-full">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {!user.hasFixedPercentage && !user.hasFixedDollarAmount && (
                    <Tag color="green">
                      Flexible: {user.calculatedPercentage?.toFixed(1)}%
                    </Tag>
                  )}
                </div>
                <div className="text-gray-500">
                  {totalAmount > 0 && <span>${amount.toFixed(2)} total</span>}
                </div>
              </div>
              <div className="flex gap-2">
                ...
                <Button danger size="small" onClick={() => removeUser(user.id)}>
                  Remove
                </Button>
              </div>
            </div>
        ...
        <Card className="mt-6 bg-gray-50">
          <div className="space-y-2">
            ...
            <div className="flex justify-between items-center">
              ...
              <Text strong>
                Remaining Amount: ${remainingDollarAmount.toFixed(2)}
              </Text>
            </div>
            {flexibleUsers.length > 0 && (
              <div className="flex justify-between items-center">
                ...
                <Text>
                  Each gets: $
                  {flexibleUsers.length > 0
                    ? (remainingDollarAmount / flexibleUsers.length).toFixed(2)
                    : "0.00"}
                </Text>
              </div>
            )}
      ...
      >
        <Form form={editForm} layout="vertical" name="edit_user_form">
          ...
          <div className="text-sm text-gray-600 mt-2">
            <strong>Note:</strong> Leave both empty for flexible split, or enter
            either percentage OR dollar amount (not both)
          </div>
        </Form>
      </Modal>
    </div>
